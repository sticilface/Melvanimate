...
function sethost(e) {
    ...
    _currentdevice = $("#text_home_device").val();
    GetData("homepage");
}
...
function RUNDOCUMENT() {
    ...
    $.post(_currentdevice + "espman/data.esp", "reboot");

});

$(document).on('click', '#upgradebutton', function() {
    $.post(_currentdevice + "espman/data.esp", "PerformUpdate=true");
    ...
    $(document).on('taphold dblclick', '.myheader', function(e) {
        ...
        $("#set_host").popup("open");
        $("#text_home_device").val(_currentdevice);
...
function processJSON(result) {
    ...
    if ($.mobile.activePage.attr('id') == 'homepage') {
        ...
        if (result.hasOwnProperty('settings')) {
            ...
            //  can maybe redo this to only cycle once and set the parameters
