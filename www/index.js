...
// $(this).children(':input[value=""]').attr("disabled", "disabled");
       ...
       $(document).on('click', '#button_add_topic', function(e) {
         $.post(_currentdevice + "data.esp", "add_topic=" + encodeURIComponent($("#input_add_topic").val()), function(data) {
         }).success(function(e) {
                   $("#status").empty().append("Success").css("color", "green");;
                   $("#input_add_topic").val("");
                   processJSON(e);
...
if(event.handled !== true) // This will prevent event triggering more then once
{
   ...
   if (nextpage.length > 0) {
     $( ":mobile-pagecontainer" ).pagecontainer( "change", "#allpresetspage", { transition: "slide" } );
