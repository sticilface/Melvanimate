...
MelvanimateMQTT::MelvanimateMQTT(Melvanimate * lights, IPAddress Addr, uint16_t Port) : _melvanimate(lights), _addr(Addr), _port(Port)
{
    ...
    char _generatedClientId[8 + 1];  // esp8266abc123
    sprintf(_generatedClientId, "_%06x", ESP.getFlashChipId());

    String temp = String(_melvanimate->deviceName()) + _generatedClientId;

    _deviceid = strdup( temp.c_str());

    _mqttClient.setClientId(_deviceid); //  disbale this so u end up with unique IDs...
